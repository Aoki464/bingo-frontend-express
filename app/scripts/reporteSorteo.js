 ...
 */
 ...
 angular.module('ReporteSorteoApp', ['ngGrid']).controller('ReporteSorteoCtrl', function ($scope, $http, $location) {
    $scope.venta = null;
    $scope.awesomeThings = [
      'HTML5 Boilerplate',
      'AngularJS',
...
//   }); 
    ...
    function respuestaListaPremiosPrincipales (largeLoad){
        
        var unePremioPrincipal = "";
          //    var aux = [];
              var respuesta = [];
              
              for(var i = 0, results = largeLoad.results.length; i<results; i++)
                  {
                      for (var j = 0, premiosPrincipales = largeLoad.results[i].premioPrincipal.length; j<premiosPrincipales;j++)
                        {
                            unePremioPrincipal = unePremioPrincipal + ", $" + largeLoad.results[i].premioPrincipal[j].nombre;
                        }
                     // aux.push(unePremioPrincipal.substring(1));
                      
                      var premioPrincipalRespuesta = {
                          sorteo: largeLoad.results[i].sorteo,
                          fechahora: largeLoad.results[i].fechahora,
                          precio: largeLoad.results[i].precio,
                          tablasVendidas: largeLoad.results[i].cartones,
                          recaudo: largeLoad.results[i].recaudo,
                          premioPrincipal: unePremioPrincipal.substring(1)
                      }
                      
                      respuesta.push(premioPrincipalRespuesta);
                      
                      unePremioPrincipal = "";
                  }
        console.log("estos son todos lo premios principales",  respuesta);
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
      setTimeout(function () {
        ...
        if (searchText) {
          ...
          $http.get('Data/reporteSorteos.json').success(function (largeLoad) {
              console.log("asdwww",largeLoad.results);
            
            data = respuestaListaPremiosPrincipales(largeLoad).filter(function(item) {
          ...
          $http.get('Data/reporteSorteos.json').success(function (largeLoad) {
            $scope.setPagingData(respuestaListaPremiosPrincipales(largeLoad),page,pageSize);
          });
        }
      }, 100);
    };
     ...
     $scope.gridOptions = {
        data:  'myData',
         //El campo Field debe coincidir con su ubicacion dentro de la respuesta Json
           columnDefs: [
         {field: 'sorteo', displayName: 'Sorteo'},
         {field: 'fechahora', displayName: 'Fecha'},
         {field: 'precio', displayName: 'Precio'},
         {field: 'tablasVendidas', displayName: 'Tablas vendidas'},
         {field: 'recaudo', displayName: 'Recaudo'},
         {field: 'premioPrincipal', displayName: 'Premios principales'},
